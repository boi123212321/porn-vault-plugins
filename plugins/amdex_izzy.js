/*
    [AmDeX] : Actor MetaData Extractor v1.4 (2020-MAY-17)

    EXPERIMENTAL VERSION COMPATIBLE WITH THE "izzy" BRANCH CURRENTLY IN DEVELOPMENT
	Updated by john4valor to work with v0.21 of Porn Vault.
	This version no longer pulls nationality data as a customField.

    This "extensible" Actor plugin currently extracts actor information from:
        - FreeOnes
            - Date of Birth IMPORTANT NOTE: See below
            - City of Birth
            - State of Birth
            - Nation of Birth
            - Actor Image
	...
	WHEN PUTTING THE CONFIG ARGUMENTS BELOW INTO YOUR CONFIG FILE,
	MAKE SURE TO DELETE 4X SPACES BEFORE EACH LINE OR PVAULT WILL THROW AN ERROR.
    If your config file is in JSON:
    "PLUGINS": {
        "AmDeX": {
            "path": "./plugins/amdex_izzy.js",
            "args": {
                "debug": false,
...
module.exports = async ({
    ...
    sourced_info.forEach(element => {
        ...
        if (args.set_dateofbirth && args.dateofbirth_source === element.id) {
            result['bornOn'] = element.born_on;
        }
		// Set Nationality
		if (args.set_nationality && args.nationality_source === element.id) {
            result['nationality'] = element.nation_of_birth;
        }
        // Set Custom Fields
        ...
        if (element.id == "freeones") {
            ...
            if (is_enabled_custom_field(element, "career_start", src_custom_fields)) {
                result['custom'][src_custom_fields.career_start] = element.extra_info.career_start;
            }
            if (is_enabled_custom_field(element, "career_end", src_custom_fields)) {
                result['custom'][src_custom_fields.career_end] = element.extra_info.career_end;
            }
        }
    ...
    async function get_info_from_freeones(actor_name, settings) {
        ...
        if (freeones_response.status == 200) {
            ...
            //Get nationality
			const nationality_first = $('[data-test="section-personal-information"] a[href*="countryCode%5D"]');
			const nationality_name = nationality_first.length ? $(nationality_first).attr("href").split("=").slice(-1)[0] : null;
			if (!nationality_name) {
				$log("[AMDX] ERR: FreeOnes No nation found for actor");
			} else {
				result["nation_of_birth"] = nationality_name;
			}
        ...
        const ethnicity_name = ethnicity_sel.length ? $(ethnicity_sel).attr("href").split("=").slice(-1)[0] : null;
        if (!ethnicity_sel) {
        ...
        const haircol_name = haircol_sel.length ? $(haircol_sel).attr("href").split("=").slice(-1)[0] : null;
        if (!haircol_name) {
        ...
        // get career info
        const career = $(".timeline-horizontal p.m-0");
        if(!career) {
            $log("[AMDX] ERR: FreeOnes No career information found");
        }
        else {
            const career_start = $(career[0]).text();
            let career_end = $(career[1]).text();
            if(career_end === "Now") {
                career_end = undefined;
            }
            if(!career_start) {
                $log("[AMDX] ERR: FreeOnes No career start information found");
            } else if(!career_end) {
                result['extra_info']["career_start"] = career_start;
            }
            else {
                result['extra_info']["career_start"] = career_start;
                result['extra_info']["career_end"] = career_end;
            }
        }
